...
type ComContract struct {
	...
	Amount              uint    `gorm:"column:amount"`                   //合同金额
	OverAmountSource    string  `gorm:"column:over_amount_source"`       //合同金额超出合约规划后 需要注明来源
	JiaStuffsAmount     uint    `gorm:"column:jia_stuffs_amount"`        //甲方供应材料金额
	UnCostAmount        uint    `gorm:"column:uncost_amount"`            //不计成本金额
	ValiSignAmount      uint    `gorm:"column:vali_sign_amount"`         //有效签约金额
