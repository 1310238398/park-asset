...
type IProjSalesHis interface {
	...
	// 创建数据
	Create(ctx context.Context, item schema.ProjSalesHis) (*schema.ProjSalesHis, error)
	// 更新数据
	Update(ctx context.Context, recordID string, item schema.ProjSalesHis) (*schema.ProjSalesHis, error)
	// 删除数据
