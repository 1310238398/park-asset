...
type IExpenditure interface {
	...
	// 创建数据
	Create(ctx context.Context, item schema.Expenditure) (*schema.Expenditure, error)
	// 更新数据
	Update(ctx context.Context, recordID string, item schema.Expenditure) (*schema.Expenditure, error)
	// 删除数据
