...
// IContractPlanningTemplate 合约规划模板业务逻辑接口
type IContractPlanningTemplate interface {
	...
	// 创建数据
	Create(ctx context.Context, item schema.ContractPlanningTemplate) (*schema.ContractPlanningTemplate, error)
	// 更新数据
	Update(ctx context.Context, recordID string, item schema.ContractPlanningTemplate) (*schema.ContractPlanningTemplate, error)
	// 删除数据
