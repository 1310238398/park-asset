...
type ProjSalesPlan struct {
	...
	ProjBusinessName string  `json:"proj_business_name" swaggo:"fales,项目业态名称"` // 项目业态名称
}
...
// FillData 填充业态名称
func (a ProjSalesPlans) FillData(projBusinList ProjBusinessFormats, m map[string]*BusinessFormat) {
	for _, item := range a {
		for _, projBusinItem := range projBusinList {
			if item.ProjBusinessID == projBusinItem.RecordID {
				businItem, ok := m[projBusinItem.RecordID]
				if !ok {
					continue
				}
				item.ProjBusinessName = businItem.Name
