...
class VisaChangeList extends PureComponent {
  ...
  save(form, key) {
    ...
    form.validateFields(async (error, row) => {
      ...
      if (index > -1) {
        ...
        response = await updatevisvaChange(row);
        if (response.record_id && response.record_id !== '') {
          message.success('更新成功');
          this.setState({ editingKey: '' });
          this.dispatch({
            type: 'visaChange/saveData',
            payload: {
              list: [...newData],
  ...
  // 签证确认页面弹出
    renVisaSureForm() {
      const {
        visaChange: { formVisibleVisaSure },
      } = this.props;
        ...
        <Row gutter={16}>
          ...
          <Col {...col}>
            <Form.Item {...formItemLayout} label="合同编号">
              {getFieldDecorator('name')(<Input placeholder="请输入合同编号" />)}
            </Form.Item>
          </Col>
          <Col {...col}>
