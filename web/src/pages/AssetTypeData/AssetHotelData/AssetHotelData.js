...
import AssetHotelDataEdit from './AssetHotelDataEdit';
...
@connect(state => ({
  assetHotelData: state.assetHotelData,
  loading: state.loading.models.assetHotelData,
}))
@Form.create()
class AssetHotelData extends PureComponent {
  state = {
    selectedRowKeys: [],
    selectedRows: [],
    project_id: '',
  ...
  handleTableChange = pagination => {
    this.dispatch({
      type: 'assetHotelData/fetchBuidings',
      pagination: {
  ...
  onItemDetailClick = item => {
    if (item.unit_num !== 0) {
      this.dispatch({
        type: 'assetHotelData/cellRoute',
        payload: item,
        unit: 1,
      });
    } else {
      this.dispatch({
        type: 'assetHotelData/floorRoute',
  ...
  handleFormBuildShow = () => {
    this.dispatch({
      type: 'assetHotelData/changeFormVisibleBuild',
      payload: false,
    });
    this.clearSelectRows();
  };
  ...
  handleFormSubmit = data => {
    const {
      assetHotelData: { formTypeBuild },
    } = this.props;
    this.dispatch({
      type: 'assetHotelData/submitBuild',
      payload: data,
    });
    if (formTypeBuild === 'E') {
      this.clearSelectRows();
  ...
  handleChangePro = e => {
    ...
    this.dispatch({
      type: 'assetHotelData/saveProjectID',
      payload: e,
    });
  };
  ...
  renderDataForm() {
    ...
    if (formTypeBuild === 'A' || formTypeBuild === 'E') {
      return <AssetHotelDataEdit onCancel={this.handleFormCancel} onSubmit={this.handleFormSubmit} />;
    }
    // if (formTypeBuild === 'S') {
    ...
    return (
      <Form onSubmit={this.handleSearchFormSubmit} layout="inline" style={{ marginBottom: '10px' }}>
        <Row gutter={16}>
          ...
          <Col {...col}>
            <Form.Item {...formItemLayout} label="公寓号">
