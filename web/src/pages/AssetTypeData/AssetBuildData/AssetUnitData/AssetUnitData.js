...
class AssetUnitMaint extends PureComponent {
  ...
  handleFormCancel = () => {
    this.dispatch({
      type: 'assetBuildData/changeFormVisibleUnit',
      payload: false,
    });
  };
  ...
  handleFormSubmit = data => {
    this.dispatch({
      type: 'assetBuildData/submitUnit',
      payload: data,
    });
  ...
  renderDataForm() {
    const {
      assetBuildData: { formTypeUnit,formDataBuild },
      location: {
        query: { currentName },
      },
    } = this.props;
    if (formTypeUnit === 'A' || formTypeUnit === 'E') {
      return (
        <AssetUnitEdit
          ...
          loudongName={formDataBuild.name}
        />
      );
    }
    // if (formTypeUnit === 'S') {
