...
import AssetPlantDataEdit from './AssetPlantDataEdit';
...
@connect(state => ({
  assetPlantData: state.assetPlantData,
  loading: state.loading.models.assetPlantData,
}))
@Form.create()
class AssetPlantData extends PureComponent {
  state = {
    selectedRowKeys: [],
    selectedRows: [],
    project_id: '',
  ...
  handleTableChange = pagination => {
    this.dispatch({
      type: 'assetPlantData/fetchPlant',
      pagination: {
  ...
  handleFormBuildShow = () => {
    this.dispatch({
      type: 'assetPlantData/changeFormVisiblePlant',
      payload: false,
    });
    this.clearSelectRows();
  };
  ...
  handleFormSubmit = data => {
    const {
      assetPlantData: { formTypePlant },
    } = this.props;
    this.dispatch({
      type: 'assetPlantData/submitPlant',
      payload: data,
    });
    if (formTypePlant === 'E') {
      this.clearSelectRows();
  ...
  handleChangePro = e => {
    ...
    this.dispatch({
      type: 'assetPlantData/saveProjectID',
      payload: e,
    });
  };
  ...
  renderDataForm() {
    ...
    if (formTypePlant === 'A' || formTypePlant === 'E') {
      return <AssetPlantDataEdit onCancel={this.handleFormCancel} onSubmit={this.handleFormSubmit} />;
    }
    // if (formTypeBuild === 'S') {
    ...
    return (
      <Form onSubmit={this.handleSearchFormSubmit} layout="inline" style={{ marginBottom: '10px' }}>
        <Row gutter={16}>
          ...
          <Col {...col}>
            <Form.Item {...formItemLayout} label="厂房号">
