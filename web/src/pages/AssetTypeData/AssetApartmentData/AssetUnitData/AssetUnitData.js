...
class AssetUnitMaint extends PureComponent {
  ...
  componentDidMount() {
    ...
    this.dispatch({
      type: 'assetApartmentData/fetchUnit',
      search: { building_type: 2, parent_id: recordID, project_id: projectID },
      pagination: {},
    });

    this.dispatch({
      type: 'assetApartmentData/fetchFormBuild',
  ...
  handleAddClick = () => {
    ...
    this.dispatch({
      type: 'assetApartmentData/LoadUnit',
      payload: {
        type: 'A',
  ...
  handleFormCancel = () => {
    this.dispatch({
      type: 'assetApartmentData/changeFormVisibleUnit',
      payload: false,
    });
  };
  ...
  handleFormSubmit = data => {
    this.dispatch({
      type: 'assetApartmentData/submitUnit',
      payload: data,
    });
  ...
  onItemDetailClick = item => {
    const {
      assetApartmentData: { formDataBuild:{name}},
    } = this.props;
    this.dispatch({
  ...
  handleDelOKClick(id) {
    this.dispatch({
      type: 'assetApartmentData/delUnit',
  ...
  renderDataForm() {
    const {
      assetApartmentData: { formTypeUnit},
      location: {
        query: { currentName },
      },
    } = this.props;
