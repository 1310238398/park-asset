...
import AssetApartmentDataEdit from './AssetApartmentDataEdit';
...
@Form.create()
class AssetApartmentData extends PureComponent {
  state = {
    selectedRowKeys: [],
    selectedRows: [],
    project_id: '',
  ...
  handleTableChange = pagination => {
    this.dispatch({
      type: 'assetApartmentData/fetchBuidings',
      pagination: {
  ...
  onItemDetailClick = item => {
    if (item.unit_num !== 0) {
      this.dispatch({
        type: 'assetApartmentData/cellRoute',
        payload: item,
        unit: 1,
      });
    } else {
      this.dispatch({
        type: 'assetApartmentData/floorRoute',
  ...
  handleAddBuildClick = () => {
    ...
    if (project_id) {
      this.dispatch({
        type: 'assetApartmentData/LoadBuild',
        payload: {
          type: 'A',
  ...
  handleFormBuildShow = () => {
    this.dispatch({
      type: 'assetApartmentData/changeFormVisibleBuild',
      payload: false,
    });
    this.clearSelectRows();
  };
  ...
  handleFormSubmit = data => {
    const {
      assetApartmentData: { formTypeBuild },
    } = this.props;
    this.dispatch({
      type: 'assetApartmentData/submitBuild',
      payload: data,
    });
    if (formTypeBuild === 'E') {
      this.clearSelectRows();
  ...
  handleDelOKClick(id) {
    this.dispatch({
      type: 'assetApartmentData/delBuild',
  ...
  handleChangePro = e => {
    ...
    this.dispatch({
      type: 'assetApartmentData/saveProjectID',
      payload: e,
    });
  };
  ...
  renderDataForm() {
    ...
    if (formTypeBuild === 'A' || formTypeBuild === 'E') {
      return <AssetApartmentDataEdit onCancel={this.handleFormCancel} onSubmit={this.handleFormSubmit} />;
    }
    // if (formTypeBuild === 'S') {
...
export default AssetApartmentData;
