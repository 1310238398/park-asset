...
class EditableCell extends React.Component {
  ...
  getInput = () => {
    ...
    if (this.props.inputType === 'number') {
      ...
      const {
        costExpenseNode: { selectListNode },
      } = this.props;
      return (
        <Select
          //mode="multiple"
...
class CostExpenseNode extends PureComponent {
  ...
  save(form, key) {
    ...
    form.validateFields(async (error, row) => {
      row.expend_rate = row.expend_rate / 100.0;
      //proj_cost_items
  ...
  mapTime(type) {
    ...
    if (index === -1 || selectListNode.length === 0) {
      return "";
    }
  ...
  getMenu = record => {
    return (
      <Menu>
        <Menu.Item onClick={() => this.brotherLevelAdd(record)}>同级添加</Menu.Item>

        <Menu.Item onClick={() => this.childLevelAdd(record)}>下级添加</Menu.Item>
      </Menu>
    );
  };
  ...
  brotherLevelAdd = currentItem => {
    ...
    const newItem = {
      ...
      parent_path:
        item.parent_path !== '' ? item.parent_path + '/' + item.record_id : item.record_id,
      name: '',
      expenditure_time_type: 1,
      expend_rate: 0,
      category: '',
  ...
  childLevelAdd = currentItem => {
    ...
    let item = this.findItem(
      data,
      currentItem.parent_path !== ''
        ? currentItem.parent_path + '/' + currentItem.record_id
