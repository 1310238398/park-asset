...
import * as assetApartmentDataService from '@/services/assetApartmentData';

export default {
  namespace: 'assetApartmentData',
  state: {
    searchBuild: {},
    paginationBuild: {},
    dataBuild: {
  ...
  effects: {
    ...
    //   const response = yield call(assetApartmentDataService.del, payload);
    ...
    *fetchFormBuild({ payload }, { call, put }) {
      const response = yield call(assetApartmentDataService.getBuildOne, payload);
      yield put({
        type: 'saveFormDataBuild',
        payload: response,
      });
    ...
    *submitBuild({ payload }, { call, put, select }) {
      ...
      if (formTypeBuild === 'E') {
        ...
        response = yield call(assetApartmentDataService.updateBuild, params);
      } else {
        response = yield call(assetApartmentDataService.createBuild, params);
      }
    ...
    *selectProjectIDName({ payload }, { call, put }) {
      const response = yield call(assetApartmentDataService.selectProInfo, payload);
    ...
    *cellRoute({ payload, unit }, { put }) {
      yield put(
        routerRedux.push({
          pathname: '/assettypedata/assetapartmentunit',
          query: {
    ...
    *delUnit({ payload }, { call, put }) {
      const response = yield call(assetApartmentDataService.delBuild, payload);
      if (response.status === 'OK') {
        message.success('删除成功');
    ...
    *floorRoute({ payload }, { put }) {
      yield put(
        routerRedux.push({
          pathname: '/assettypedata/assetapartmentfloor',
          query: {
    ...
    *plateRoute({ payload }, { put }) {
      yield put(
        routerRedux.push({
          pathname: '/assettypedata/assetapartmentnumplate',
          query: {
            recordID: payload.item.record_id,
            projectID: payload.item.project_id,
            type: payload.item.asset_type,
    ...
    *subDoorRoute({ payload }, { put }) {
      yield put(
        routerRedux.push({
          pathname: '/assettypedata/assetapartmentsubdoor',
          query: {
            recordID: payload.record_id,
            projectID: payload.project_id,
    ...
    *fetchBuidings({ search, pagination }, { call, put, select }) {
      ...
      const response = yield call(assetApartmentDataService.queryBuildingsPage, params);
      yield [
        put({
          type: 'saveBuidings',
          payload: response,
    ...
    *submitPlate({ payload }, { call, put, select }) {
      ...
      if (response.record_id && response.record_id !== '') {
        ...
        yield put({
          type: 'fetchFormPlate',
          search:{project_id:proid,record_id: payload.parent_id}
        });
      }
    },
