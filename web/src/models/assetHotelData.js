...
import * as assetHotelDataService from '@/services/assetHotelData';

export default {
  namespace: 'assetHotelData',
  state: {
    searchBuild: {},
    paginationBuild: {},
    dataBuild: {
  ...
  effects: {
    ...
    //   const response = yield call(assetHotelDataService.del, payload);
    ...
    *fetchFormBuild({ payload }, { call, put }) {
      const response = yield call(assetHotelDataService.getBuildOne, payload);
      yield put({
        type: 'saveFormDataBuild',
        payload: response,
      });
    ...
    *submitBuild({ payload }, { call, put, select }) {
      ...
      if (formTypeBuild === 'E') {
        ...
        response = yield call(assetHotelDataService.updateBuild, params);
      } else {
        response = yield call(assetHotelDataService.createBuild, params);
      }
    ...
    *selectProjectIDName({ payload }, { call, put }) {
      const response = yield call(assetHotelDataService.selectProInfo, payload);
    ...
    *cellRoute({ payload, unit }, { put }) {
      yield put(
        routerRedux.push({
          pathname: '/assettypedata/assethotelunit',
          query: {
    ...
    *delUnit({ payload }, { call, put }) {
      const response = yield call(assetHotelDataService.delBuild, payload);
      if (response.status === 'OK') {
        message.success('删除成功');
    ...
    *floorRoute({ payload }, { put }) {
      yield put(
        routerRedux.push({
          pathname: '/assettypedata/assethotelfloor',
          query: {
    ...
    *plateRoute({ payload }, { put }) {
      yield put(
        routerRedux.push({
          pathname: '/assettypedata/assethotelnumplate',
          query: {
            recordID: payload.item.record_id,
            projectID: payload.item.project_id,
            type: payload.item.asset_type,
    ...
    *subDoorRoute({ payload }, { put }) {
      yield put(
        routerRedux.push({
          pathname: '/assettypedata/assethotelsubdoor',
          query: {
            recordID: payload.record_id,
            projectID: payload.project_id,
    ...
    *fetchBuidings({ search, pagination }, { call, put, select }) {
      ...
      const response = yield call(assetHotelDataService.queryBuildingsPage, params);
      yield [
        put({
          type: 'saveBuidings',
          payload: response,
