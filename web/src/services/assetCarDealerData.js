...
const buildingsRouter = 'car_changers';

export async function queryCarDealerPage(params) {
  return request(`/v1/${buildingsRouter}?${stringify(params)}`);
}

// 查询单条数据
export async function getCarDealerOne(params) {
...
// 更新保存
export async function updateCarDealer(params) {
  return request(`/v1/${buildingsRouter}/${params.record_id}`, {
    method: 'PUT',
    body: params,
  });
...
// 创建
export async function createCarDealer(params) {
...
// 删除
export async function delCarDealer(params) {
  return request(`/v1/${buildingsRouter}/${params.record_id}`, {
    method: 'DELETE',
  });
}
